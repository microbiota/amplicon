# 样本或组的功能组成堆叠柱状图 Stackplot of function for samples and groups
#
# This is the function named 'func_stackplot'
# which draw stack plot, and return a ggplot2 object
#
#' @title Plotting stackplot of function for groups or samples
#' @description Input function composition, and metadata (SampleID and groupID). Then select top N high abundance function and group other low abundance. When Select samples can draw sample composition by facet groups. If used group can show mean of each group. Finally, return a ggplot2 object.
#' @param func_sum composition matrix, like OTU table and rowname is function, typical output of usearch -sinfunc_summary;
#' @param metadata matrix or dataframe, including sampleID and groupID;
#' @param topN Top N function to show, default 8, alternative 4, 6, 10 ...;
...
#' @export
func_stackplot <- function(func_sum, metadata, topN = 8, groupID = "Group", style = "group", sorted = "abundance") {
  ...
  # 交叉筛选
  idx = rownames(metadata) %in% colnames(func_sum)
  metadata = metadata[idx,]
  func_sum = func_sum[, rownames(metadata)]
  ...
  # 按丰度降序排序
  mean_sort = as.data.frame(func_sum[(order(-rowSums(func_sum))), ])
  ...
  # 保存变量备份，并输出至文件
  merge_func=mean_sort
  
  if (style == "sample"){
    # 添加功能并设置排序方式，默认字母，abundance按丰度
    mean_sort$Function = rownames(mean_sort)
    data_all = as.data.frame(melt(mean_sort, id.vars=c("Function")))
    if (sorted == "abundance"){
      data_all$Function  = factor(data_all$Function, levels=rownames(mean_sort))
    }
    # set group facet
    ...
    p = ggplot(data_all, aes(x=variable, y = value, fill = Function )) +
